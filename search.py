...
class Problem:
    ...
    def __init__(self, initial, goal=None, expandnode=None, prof=None):
...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if depth < len(node.path()):
            depth = len(node.path())
        if problem.goal_test(node.state):
            expandnodes = len(fringe)
            problem.expandnode=expandnodes
            problem.prof=depth
            #print("Cerrada", closed)
            return node
...
def graph_search_min_heuristic(problem, fringe):
    """Search through the successors of a problem to find a goal.
    The argument fringe should be an empty queue.
    If two paths reach a state, only use the best one. [Fig. 3.18]"""
    problem.expandnode = 0
...
def min_value_graph_search(problem):
    return graph_search_min_heuristic(problem, MinCostQueue())


def min_value_graph_search_heuristic(problem):
    return graph_search_min_heuristic(problem,MinCostQueueHeuristic())
