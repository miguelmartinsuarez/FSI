...
class MinCostQueue(Queue):
    ...
    def sort(self,problem):
        self.A.sort(key=lambda x: x.path_cost)
        #print self.A
        #print self.A[self.start].path_cost



class MinCostQueueHeuristic(Queue):
    ...
    def extend(self, items, problem):
        ...
        #print problem.h(self.A[self.start])

        # self.A.sort(key=lambda x: x.path_cost +x.problem.h(x))
        #print self.A
